"""
按子分类拆成独立文件，每个子分类是一个独立数据源
这样随机池可以单独加「品质保证」「光影效果」各抽各的
"""
import json
...
STRUCTURE = {
    ...
    "动作姿态": {
        ...
        "全身动作": [
            ...
            "spinning", "twirling", "kicking", "punching",
...
for f in DATA_DIR.glob("*.json"):
    name = f.name
    if name not in ("lora角色.json",):
        f.unlink()
        print(f"  删旧: {name}")

# 清理旧目录
for d in DATA_DIR.iterdir():
    if d.is_dir() and d.name in STRUCTURE:
...
for main_cat, subs in STRUCTURE.items():
    ...
    for sub_name, tags in subs.items():
        ...
        fpath = cat_dir / f"{sub_name}.json"
        with open(fpath, "w", encoding="utf-8") as f:
            json.dump(entries, f, ensure_ascii=False, indent=2)
        n = len(entries)
        total += n
        file_count += 1
        print(f"  {main_cat}/{sub_name}.json  ({n}条)")

print(f"\n✅ 共 {file_count} 个源, {total} 条标签")
print("现在每个子分类是独立源，随机池里可以分别添加")
